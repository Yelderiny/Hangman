...
public class Hangman {
    ...
    public boolean targetContains(String target, char letter)
    {
        ...
        {
            if (letter == target.charAt(i)) return true; //if there is a match return true
        }
     ...
     */
    public boolean hasFoundTarget(String target, String guess) { return target.equals(guess); } //returns true if the target and the guess are the same
    ...
    public String getCurrentGuessForDisplay(String currentGuess)
    {
        ...
        {
            String goodLetter; //string to identify letter

            if (Character.isLetter(currentGuess.charAt(i))) goodLetter = "" + currentGuess.charAt(i); //if the char at i is a letter, initialize goodLetter with that char
            else goodLetter = "_"; //if an *, add an underscore

            out.append(goodLetter).append(" "); //add goodLetter to out
        }

        return out.toString().toUpperCase(); //convert out to string and make uppercase
    }
    ...
    {
        //check if the letter is contained in the target
        if (targetContains(target, letter))
        {
            ...
            for (int i = 0; i < target.length(); i++)
            {
                if (letter == target.charAt(i)) out.append(letter); //add the letter if found in the target
                else out.append(guess.charAt(i)); //otherwise, add the current character in the guess
            }
            return out.toString(); //return the concatenated String
        }
        else return guess; //if letter is not found, return guess
    }


    /******************************************************************************
     You do not need to look at the code be low this point. If you are curious, however, feel
